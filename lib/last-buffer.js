...
export default {
  ...
  activate() {
    ...
    this.subscriptions.add(atom.workspace.onDidDestroyPaneItem(({ pane: { id: paneId }, item: { id: editorId } }) => {
      _.remove(this.items, ({ pane: { id: _paneId }, editor }) => paneId === _paneId && _.get(editor, 'id') === editorId)
    }))
    this.subscriptions.add(atom.workspace.onDidDestroyPane(({ pane: { id: paneId, items: editors } }) => {
      _.remove(this.items, ({ pane: { id: _paneId }, editor }) => paneId === _paneId && _.find(editors, ({ id: editorId }) => _.get(editor, 'id') === editorId))
    }))
    this.subscriptions.add(atom.workspace.onDidStopChangingActivePaneItem(editor => {
